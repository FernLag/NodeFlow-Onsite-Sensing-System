...
\begin{description}[leftmargin=1cm,style=nextline]
  ...
  \item[A battery warning appears while on USB]
        Ignore it. The battery check measures the supply against a reference
        inside the chip, which only means something when the board is running
        from a battery through the round barrel socket. On USB power, including
        a USB power bank, it cannot tell.
